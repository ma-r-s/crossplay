...
refuses the file outright on a key that matches no puzzle, a character outside
`A-Z 0-9`, space, hyphen and apostrophe, or a name over 9 characters -- every one
a hard error, because a half-applied annotation pass looks exactly like a full
one.

**Nine characters is the cap and it is measured, not guessed.** The band is
