...
| Piece         | How                                                                                                                                                                                                                                                                           | Status                          |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| .apkg opening | JSZip; `collection.anki21b` is zstd (fzstd) + sql.js, legacy `.anki2` plain                                                                                                                                                                                                   | known-good libs, needs wiring   |
...
| Fonts         | FreeType mono rasterisation is native (`freetype-py`); needs a small C shim compiled to wasm reproducing `mono_cpfont.py`'s loop. **Acceptance test: byte-identical .cpfont output vs the CLI on the same input.**                                                            | the long pole, bounded          |
| Preview       | emulator `crossplay.data` mounts at `/fs_` (MEMFS, writable at runtime). Page writes `/fs_/study/<deck>/…` via `Module.FS` and relaunches Study. May need `FS` in EXPORTED_RUNTIME_METHODS and a JS hook to re-enter the app; we own the build (`tools_local/wasm/build.py`). | spiked 2026-08-10               |
| Write to card | File System Access directory handle; zip fallback                                                                                                                                                                                                                             | standard                        |
| Review sync   | `deck_to_anki.py`'s apply() in Pyodide against sql.js? No: sqlite3-in-Pyodide can open a user-provided file copy directly; write-back through the file handle with a backup sibling                                                                                           | exists, needs the file plumbing |
