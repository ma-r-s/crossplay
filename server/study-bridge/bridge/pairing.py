...
Codes are 8 chars from an unambiguous alphabet (no 0/O/1/I), expire in five
minutes, and are single-use. The device token returned by a successful poll
is 32 random bytes; the store keeps only its hash. Both directions of the
pairing race are closed elsewhere: claim is CSRF-guarded (app.py) and the
device shows "Paired to <username> -- confirm?" before persisting anything
(firmware slice).
"""
...
class Pairings:
    """In-memory: a pi reboot forgets pending pairings, which costs the user
    one more QR scan and nothing else. Paired tokens live in state.json."""
