#!/usr/bin/env bash
# Post-deploy isolation regression for ankibridge: the walls must hold and the
...
probe() {
  ...
  docker exec ankibridge python -c "
import socket, sys
s = socket.socket()
s.settimeout(3)
try:
...
"
  ...
  case $rc in
    ...
    *) echo "FAIL docker exec exited $rc; is the ankibridge container running?"; fail=1 ;;
  esac
}
...
if docker exec ankibridge python -c \
    "import socket; socket.getaddrinfo('ankiweb.net', 443)"; then
  echo "ok   DNS resolves ankiweb.net"
else
...
req = urllib.request.Request('https://ankiweb.net', method='HEAD')
try:
    urllib.request.urlopen(req, timeout=10)
except urllib.error.HTTPError:
    pass   # any HTTP status is still proof the internet is reachable
"; then
  echo "ok   HTTPS HEAD to ankiweb.net"
else
  echo "FAIL outbound HTTPS to ankiweb.net is broken"; fail=1
fi
