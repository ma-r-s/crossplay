#!/usr/bin/env bash
# Confine the ankibridge containers to the public internet only.
#
...
# qBittorrent, Prowlarr, SSH and the router. It holds AnkiWeb credentials and
# user collections and is reachable from the internet, so it is treated as a
...
# 172.31.83.0/24 is getbooks; ankibridge has its own pinned /24 (compose.yaml).
SUBNET="172.31.84.0/24"
TAG="ankibridge-isolation"
...
pos=1
# Siblings first: cloudflared has to reach ankibridge, and both live inside
...
echo "ankibridge isolation applied for $SUBNET"
