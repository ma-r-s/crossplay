#!/usr/bin/env bash
# Confine the packs containers to the public internet only.
#
# Docker's default bridge gives a container free rein over the host and the
# LAN: before these rules the service could open Immich, Jellyfin, qBittorrent,
# Prowlarr, SSH and the router. It parses hostile input from the internet, so it
# is treated as a machine that will eventually be compromised.
#
...
pos=1
# Siblings first: cloudflared has to reach packs, and both live inside the
# 172.16/12 block that the next rules drop.
...
echo "packs isolation applied for $SUBNET"
