...
| `src/main.cpp`, `src/CrossPointSettings.h`, `src/SettingsList.h`, `src/activities/settings/OtaUpdateActivity.cpp` | The daily heartbeat's hooks: `heartbeat::begin()`/`update()` beside the Developer Mode calls, one settings field and its row, and the OTA attempt/failure notes; the module itself is `src/network/Heartbeat*.{h,cpp}`. See `docs/workflow/events.md` | 2 calls + 1 field + 1 row + 2 calls |
