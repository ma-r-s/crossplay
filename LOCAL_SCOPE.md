...
| `src/main.cpp`, `src/CrossPointSettings.h`, `src/SettingsList.h`, `src/activities/settings/OtaUpdateActivity.cpp`, `src/activities/settings/SdFirmwareUpdateActivity.cpp` | The daily heartbeat's hooks: `heartbeat::begin()`/`update()` beside the Developer Mode calls, one settings field and its row, and the install attempt/failure notes on both install screens (the SD one is the OTA one's twin); the module itself is `src/network/Heartbeat*.{h,cpp}`. See `docs/workflow/events.md` | 2 calls + 1 field + 1 row + the notes on 2 screens |
