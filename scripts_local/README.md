...
| `check.sh`       | Everything verifiable without a device: every host suite, then both builds. Run before every commit. `--tests` for suites only. `--committed` verifies HEAD in a throwaway worktree instead of your working tree. Reports each suite's own exit code, not just its last line.                                                                                            |
