...
int main() {
  ...
  for (const char* yes : {"TAP 1 1", "LONG 1 1", "SWIPE 1 1 2 2", "BTN UP"}) {
    if (devinput::isCommand(yes)) ok(); else bad(std::string("isCommand rejected ") + yes);
  }
  ...
  for (const char* no : {"PING", "TAP", "SCREENSHOT", "", "BTNX UP", "tap 1 1"}) {
    if (!devinput::isCommand(no)) ok(); else bad(std::string("isCommand claimed ") + no);
  }
  ...
  if (seen.ms == 140) ok(); else bad("tap default hold");
  ...
  if (seen.ms == 250) ok(); else bad("swipe default ms");
  ...
  // -- BTN ------------------------------------------------------------------
  struct { const char* name; int index; } buttons[] = {
  ...
  for (const auto& b : buttons) {
    ...
    if (seen.button == b.index) ok(); else bad(std::string("BTN ") + b.name + " mapped to " + std::to_string(seen.button));
  }
  expect("BTN NOPE", "ERR BTN wants");
  expect("BTN UP 500", "OK BTN UP 500");
  if (seen.ms == 500) ok(); else bad("btn explicit hold");
  ...
  if (seen.calls == before) ok(); else bad("a refused command still reached the injector");
