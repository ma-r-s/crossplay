...
if ! psql < "$HERE/prelude.sql" > /tmp/relwatch.prelude 2>&1; then
  echo "FAIL relwatch  the prelude did not apply"; cat /tmp/relwatch.prelude; exit 1
fi

for m in "$MIG"/*.sql; do
  if ! sed -e '/^create extension if not exists pg_net/s/^/-- test: /' \
           -e '/^create extension if not exists pg_cron/s/^/-- test: /' "$m" \
       | psql > /tmp/relwatch.mig 2>&1; then
    echo "FAIL relwatch  migration $(basename "$m") did not apply"
    tail -20 /tmp/relwatch.mig
    exit 1
  fi
done

if ! psql < "$HERE/checks.sql" > /tmp/relwatch.checks 2>&1; then
  echo "FAIL relwatch  the checks did not run to the end"
  tail -30 /tmp/relwatch.checks
  exit 1
fi
