...
# The release is built once per tag. crossplay-release.yml fires on a tag
# pushed with RELEASE_TOKEN, and the autorelease dispatches it for a tag
# pushed with the workflow's own token, which starts nothing on its own.
# Both at once built and published v1.12.16 twice on 2026-09-04, and the
# board holds two release events for it. The step's own text runs here with
# a fake gh that records every call.
...
publish() {  # label, PUSH_STARTS_WORKFLOWS value, wanted number of dispatches
  rm -f "$WORK/gh.calls"
  ( cd "$WORK" && PATH="$WORK/bin:$PATH" PUSH_STARTS_WORKFLOWS="$2" NEXT=1.2.3 bash -eo pipefail publish.sh >"$WORK/out" 2>&1 )
