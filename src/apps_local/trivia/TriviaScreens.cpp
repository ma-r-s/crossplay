...
void chrome(toybox::Screen& screen, const char* title, const char* rightLabel) {
  ...
  if (rightLabel != nullptr && *rightLabel != '\0') {
    // The band's VISIBLE rows, matching headerBand()'s own centring. Boxed over
    // the whole band instead, the label centres partly in rows the bezel covers
    // and rides above the title it is meant to sit beside. Asked for rather
    // than rebuilt from the content top, which was a copy of the chrome's
    // geometry kept in an app file.
