...
constexpr Terrain buildCityOfClouds() {
  ...
  const uint16_t xs[16] = {13, 1000, 993, 8, 16, 993, 984, 0, 13, 997, 502, 509, 507, 495, 493, 494};
  const uint16_t ys[16] = {79, 90, 296, 293, 505, 509, 706, 700, 901, 916, 186, 389, 593, 791, 1000, 0};
  for (int i = 0; i < 16; ++i) {
    t.x[i] = xs[i];
    t.y[i] = ys[i];
  }
  ...
  const R regions[] = {
      ...
      {(1u << 3) | (1u << 4) | (1u << 11), 1},
      {(1u << 0) | (1u << 3) | (1u << 10), 1},
      {(1u << 2) | (1u << 3) | (1u << 10) | (1u << 11), 2},
      {(1u << 4) | (1u << 5) | (1u << 11) | (1u << 12), 2},
      {(1u << 6) | (1u << 7) | (1u << 12) | (1u << 13), 2},
      {(1u << 8) | (1u << 9) | (1u << 13), 1},
  };
  t.regionCount = static_cast<uint8_t>(sizeof(regions) / sizeof(regions[0]));
  for (int i = 0; i < t.regionCount; ++i) {
