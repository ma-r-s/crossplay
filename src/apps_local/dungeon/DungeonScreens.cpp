...
Layout layoutBoard(const fui::Rect& body, const int size) {
  Layout layout;
  layout.cell = kCell;
  layout.lane = kLane;
  layout.size = static_cast<int16_t>(size);
  // Lane, then the gap the frame lives in, then the cells.
  const int extent = kLane + kClueGap + size * kCell;
  ...
  layout.board = fui::makeRect(static_cast<int16_t>(x + kLane + kClueGap), static_cast<int16_t>(y + kLane + kClueGap),
                               static_cast<int16_t>(size * kCell), static_cast<int16_t>(size * kCell));
  return layout;
}
...
void buildBoard(toybox::Screen& screen, const BoardModel& model, Layout& layout) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    const fui::Rect colBox =
        fui::makeRect(static_cast<int16_t>(layout.board.x + i * layout.cell + kLaneOverhang),
                      static_cast<int16_t>(layout.board.y - kClueGap - kLane), static_cast<int16_t>(kLane),
                      static_cast<int16_t>(kLane));
...
void buildMenu(toybox::Screen& screen, const MenuModel& model) {
  ...
  if (model.progress != nullptr) {
    ...
    const fui::Rect tutorial = fui::makeRect(static_cast<int16_t>(body.x + (body.width - pip) / 2),
                                             static_cast<int16_t>(top), static_cast<int16_t>(pip),
                                             static_cast<int16_t>(pip));
  ...
  const int width = (actions.width - toybox::kGutter) / 2;
  const char* labels[2] = {"PLAY", "CHOOSE"};
  const int values[2] = {ButtonPlay, ButtonChoose};
  for (int i = 0; i < 2; ++i) {
    fui::ButtonProps props;
...
void mapCell(toybox::Screen& screen, const fui::Rect& box, const int index, const bool done, const bool current) {
  ...
  if (current) {
    screen.target().stroke(box.inset(fui::Insets{-5, -5, -5, -5}), fui::Paint::solid(fui::Color::Black),
                           toybox::kRule, 10);
  }
}
...
void buildPicker(toybox::Screen& screen, const PickerModel& model, PickerLayout& layout) {
  ...
  caption.font = toybox::kSmallFont;
  ...
  constexpr int gap = 5;
  constexpr int tutorialSize = 44;
  constexpr int tutorialCaption = 22;
  const fui::Rect body = screen.body();

  // The tutorial and the map are centred together, as one block. Reserving the
  // tutorial off the top first and centring the grid in the remainder put two
  // thirds of the slack under the caption and left the screen looking like it
  // had a hole in it.
  int cell = (body.width - lane - 7 * gap) / 8;
  const int blockExtra = tutorialCaption + tutorialSize + 2 * toybox::kGutter;
  if (cell > (body.height - blockExtra - 7 * gap) / 8) cell = (body.height - blockExtra - 7 * gap) / 8;
  const int gridExtent = 8 * cell + 7 * gap;
  const int blockWidth = lane + gridExtent;
  const int blockHeight = blockExtra + gridExtent;
  const int left = body.x + (body.width - blockWidth) / 2;
  const int top = body.y + (body.height - blockHeight) / 2;

  // The tutorial is not part of the eight-by-eight, so it sits above it rather
  // than being squeezed into a row it does not belong to.
  screen.target().text(fui::makeRect(body.x, static_cast<int16_t>(top), body.width, tutorialCaption), "TUTORIAL",
                       caption);
  const fui::Rect tutorial = fui::makeRect(static_cast<int16_t>(body.x + (body.width - tutorialSize) / 2),
                                           static_cast<int16_t>(top + tutorialCaption + toybox::kGutter),
                                           tutorialSize, tutorialSize);
  screen.frame().hit(tutorial.inset(fui::Insets{-10, -10, -10, -10}), ActionPick, 0);
  mapCell(screen, tutorial, 0, model.progress != nullptr && model.progress->isSolved(0), model.current == 0);

  layout.grid = fui::makeRect(static_cast<int16_t>(left + lane), static_cast<int16_t>(top + blockExtra),
  ...
  for (int tier = 0; tier < 8; ++tier) {
    ...
    const fui::Rect chip = fui::makeRect(static_cast<int16_t>(left), rowY, static_cast<int16_t>(lane - 6),
  ...
  const fui::Rect artBox = fui::makeRect(static_cast<int16_t>(nextBand.x),
                                         static_cast<int16_t>(nextBand.y + (nextBand.height - nextArt) / 2), nextArt,
                                         nextArt);
...
void buildWin(toybox::Screen& screen, const WinModel& model) {
  ...
  if (model.cleared != nullptr) {
    ...
    screen.target().stroke(fui::makeRect(static_cast<int16_t>(mapX - toybox::kFrame),
                                         static_cast<int16_t>(mapY - toybox::kFrame),
